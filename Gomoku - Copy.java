...
public class Gomoku extends Application {
  /** An array of integers which describe X-directions in the following order: right, left, down, up, down-right, up-left, down-left, up-right */
  private static final int dirX[] = new int[]{0, 0, 1, -1, 1, -1, 1, -1};
  
  /** An array of integers which describe Y-directions in the following order: right, left, down, up, down-right, up-left, down-left, up-right */
  ...
  public static boolean isFourFour(int[][] board, int idRow, int idColumn, int numNeedToWin) {
    for(int dir = 0; dir < 8; dir++) {
      for(int curRow = idRow, curColumn = idColumn;
          isValid(board, curRow, curColumn) && board[curRow][curColumn] == board[idRow][idColumn];
          curRow += dirX[dir], curColumn += dirY[dir]) {
            // Store the number of groups that have 4 pieces (in 4 directions: horizontal, vertical, two diagonals)
            int count = 0;
            for(int i = 0; i < 4; i++) {
              // Store the total same pieces in just one direction
              int totalSamePiece = numberInLine(board, curRow, curColumn, i * 2) + numberInLine(board, curRow, curColumn, i * 2 + 1) - 1;
      ...
      for(int curRow = idRow, curColumn = idColumn;
          ...
          curRow += dirX[dir], curColumn += dirY[dir]) {     
            ...
            for(int i = 0; i < 4; i++) {
              ...
              int same1 = numberInLine(board, curRow, curColumn, i * 2) - 1;
              // Store the total same pieces in another way of the 'direction'
              int same2 = numberInLine(board, curRow, curColumn, i * 2 + 1) - 1;
              if (same1 + same2 + 1 == numNeedToWin - 2
                    && isOpen(board, curRow, curColumn, i * 2)
                    && isOpen(board, curRow, curColumn, i * 2 + 1))
                ++count;
            }
            if (count >= 2)
              return true;
  ...
  public void start(Stage stage) {
    ...
    /** 
     * Extra: Adding two more features below the table to inform users of currentPlayer and the game message 
    ...
    notificationBar.setMinWidth(Math.max(300, stage.getWidth() - 60));
    ...
    borderPane.setLeft(showCurrentPlayer);
    borderPane.setRight(notificationBar);
    borderPane.setTop(pane);
    BorderPane.setAlignment(pane, Pos.CENTER);
    pane.setAlignment(Pos.CENTER);
    BorderPane.setAlignment(showCurrentPlayer, Pos.CENTER);
    BorderPane.setMargin(showCurrentPlayer, new Insets(0, 10, 10, 10));
    ...
    try {
      switch (args.length) {
        ...
        case 3:
          ...
          break;
        default:
          System.out.println("You must input an appropriate length (<= 3) of arguments");
      }
